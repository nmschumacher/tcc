...
def learning_curves_and_metrics(X_train, y_train, X_cv, y_cv, X_test, y_test, model, step=55, dataset='TREC 3k', etapa='E1'):
	...
	_etapas_common(model, errors, X_test, y_test, dataset, etapa=etapa)
...
def etapa4():
	...
	for load_dataset in load_datasets_functions:
		...
		for idx in [48,50,55,59,60,61]: # total_features:
			print('\nExtraindo features...')
			start_time = time.time()
			X_f, added_feature = _etapa4(X_text, X, idx)
		...
		df.to_csv('resultados E4 ' + load_dataset[1] + '.csv', index=False)


def _etapa4_tf_idf():
	X_f, added_feature = _etapa4(X_text, X_tfidf, idx)
...
def _etapas_common(model, errors, X_test, y_test, dataset='TREC 3k', etapa='E1'):
	...
	print('F1-score treino:    %.5f' %(1-errors['train'][len(errors['train']) - 1]))
	print('F1-score validação: %.5f' %(1-errors['cv'][len(errors['cv']) - 1]))
	_print_all_metrics(calc.all_metrics(model, X_test, y_test))
